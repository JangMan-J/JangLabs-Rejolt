...
- P2: Frontmatter dialect: the constrained-YAML subset frozen per Appendix B2 (accept AND reject surfaces enumerated — the builder implements the boundary, never invents it) + the hand-rolled parser; oracle per A3+B2: differential agreement with the named reference parser on the in-subset corpus, generate→parse→regenerate round-trip, §3 examples + B2 rejection cases as the fixture corpus; deny diagnostics cite the violated dialect rule ← D21, A3, A8

...
- R1. Perf budget + regression-slack magnitudes — resolve at `/build` via the D26 calibration protocol; `/vet` verifies the calibration commit exists and followed the derivation rule.
- R2. Flat recall-index column schema (field order, escaping, snippet truncation) — resolve at `/blueprint` under D24.
- R3. Normalized-operation field-level schema (serde struct definitions; per-tool extraction table incl. Bash segment/privilege/env-var handling carried from the extraction findings) — resolve at `/blueprint` under D19.
- R4. `CORE-SPEC.md` v2 sync — fold this ledger's supersessions (D15–D26) into the spec doc; `/blueprint` decides whether spec-sync is a work packet or ship-stage compaction (G6 either way).
- R5. Grammar seed vocabulary — which tags (if any) seed `grammar.toml` at first bootstrap beyond the empty seed; owner call, schedule at `/blueprint`.
- R6. Claude Code hook wiring (which hook events → which `rejolt` modes; settings placement; install manifest boundary) — resolve at `/blueprint` under D19/D13.
- R7. Curation/telemetry parameter carry-over audit (which §8/§10 tunables carry unchanged vs recalibrate) — resolve at `/blueprint`; defaults carry from CORE-SPEC unless evidence says otherwise.
